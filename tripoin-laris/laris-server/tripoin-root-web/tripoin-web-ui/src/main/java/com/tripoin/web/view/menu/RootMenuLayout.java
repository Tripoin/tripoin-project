...
import com.tripoin.web.view.style.IcoMoon;
...
public class RootMenuLayout extends HorizontalLayout implements View {
	...
	private Component buildTitle() {
        ...
        final Label title = new Label("<h3>Web Application <strong>Laris</strong></h3>", ContentMode.HTML);
	...
	private Component buildMenuItems() {
		...
		for(final MenuData menuData : menuDatas){
			if("profile".equals(menuData.getCode()))
				continue;
			if(menuData.getLevel() == 1 && ParameterConstant.MENU_NON_PAGE.equals(menuData.getFunction())){
